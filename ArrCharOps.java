...
public class ArrCharOps {
    ...
    public static char[] concat(char[] arr1, char[] arr2) {
        char bigArry[] = new char[arr1.length + arr2.length];
        for (int i = 0; i < arr1.length; i++) {
            bigArry[i] = arr1[i];
        } // end to add first arr1
        for (int i = arr1.length; i < bigArry.length; i++) {
            bigArry[i] = arr2[i - arr1.length];
        } // end to add second arr2
        return bigArry;
    }
    ...
    public static char[] subArray(char[] arr, int beginIndex, int endIndex) {
        if (beginIndex < 0 ||
                ...
                endIndex > arr.length)
            return null;
        char result[] = new char[endIndex - beginIndex];// declear 'result' arry size of subArry
    ...
    public static long hashCode(char[] arr) {
        long wow = 0;
        for (int i = 0; i < arr.length; i++) {
            long power = (long) Math.pow(7, (arr.length - (i + 1))); // we want to start deduce from 1 and not 0=i
            wow += arr[i] * power;
        }
        return wow;
    }
    ...
    public static int compareTo(String str1, String str2) {
        ...
        int smallerSize = Math.min(str1.length(), str2.length());// want to run on the smaller string
        //int smallerSize = (str1.length() <= str2.length()) ? str1.length() : str2.length();
        int temp = -2;

        for (int i = 0; i < smallerSize; i++) {
            if (str1.charAt(i) != str2.charAt(i)) {// if mot equal
                if (str1.charAt(i) < str2.charAt(i))
                    temp = -1;// if str1 is lexicographically less than str2
                else {
                    temp = 1; // else if str1 is lexicographically greater than str2
                }
                break;
            }
            temp = 0; // if they are equal
        }
        if (temp == 0 && str1.length() != str2.length()) { // if until now equal but different size
            if (str1.length() == smallerSize)
                temp = -1;
            else
                temp = 1;
        }
